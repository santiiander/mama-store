...
const Utils = {
    ...
    convertGoogleDriveUrl: (url) => {
        ...
        if (cleanUrl.length > 20 && cleanUrl.length < 50 && !cleanUrl.includes('http') && !cleanUrl.includes('.')) {
            ...
            console.log('✅ Convertido desde ID (thumbnail):', convertedUrl);
            return convertedUrl;
        }
        ...
        if (cleanUrl.includes('drive.google.com/file/d/')) {
            ...
            if (fileId) {
                ...
                console.log('✅ Convertido desde URL completa (thumbnail):', convertedUrl);
                return convertedUrl;
            }
        }
        ...
        if (cleanUrl.includes('drive.google.com/open?id=')) {
            ...
            if (fileId) {
                const convertedUrl = `https://drive.google.com/thumbnail?id=${fileId[1]}&sz=w400-h300`;
                console.log('✅ Convertido desde URL open (thumbnail):', convertedUrl);
                return convertedUrl;
            }
        }
        ...
        if (cleanUrl.includes('drive.google.com/thumbnail')) {
            console.log('✅ URL ya en formato thumbnail');
            return cleanUrl;
        }
        ...
        if (cleanUrl.includes('drive.google.com/uc?export=view')) {
            ...
            if (fileId) {
                const convertedUrl = `https://drive.google.com/thumbnail?id=${fileId[1]}&sz=w400-h300`;
                console.log('✅ Convertido desde uc a thumbnail:', convertedUrl);
                return convertedUrl;
            }
            console.log('⚠️ URL uc sin ID válido');
            return cleanUrl;
        }
        
        // Si es otra URL válida (http/https), devolverla tal como está
        if (cleanUrl.startsWith('http')) {
            console.log('✅ URL externa válida');
            return cleanUrl;
        }
        
        console.log('⚠️ URL no reconocida, devolviendo original');
        return cleanUrl;
    },

    // Parsear CSV de Google Sheets
    parseCSV: (csvText) => {
        console.log('🔍 Parseando CSV completo:', csvText);
        ...
        if (lines.length < 2) {
            console.log('⚠️ No hay suficientes líneas para procesar');
            return [];
        }
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (line) {
                ...
                console.log(`🔢 Valores extraídos de línea ${i}:`, values);
                ...
                console.log(`📦 Objeto creado para línea ${i}:`, row);
                ...
                if (nombreProducto.trim() && precioProducto.trim()) {
                    console.log(`✅ Producto válido encontrado: ${nombreProducto}`);
                    data.push(row);
                } else {
                    console.log(`❌ Producto inválido descartado - Nombre: "${nombreProducto}", Precio: "${precioProducto}"`);
                }
            } else {
                console.log(`⚠️ Línea ${i} está vacía, saltando...`);
            }
        }
        
        console.log('✅ Datos finales parseados (solo productos válidos):', data);
        return data;
    },
...
const ProductManager = {
    ...
    loadProducts: async () => {
        ...
        try {
            console.log('🔄 Cargando productos desde Google Sheets:', API_CONFIG.GOOGLE_SHEETS_CSV_URL);
            ...
            AppState.products = data
                ...
                .map(item => {
                    ...
                    console.log('🖼️ Procesando imagen para producto:', item['Nombre producto']);
                    console.log('  - URL original:', originalImageUrl);
                    console.log('  - URL convertida:', convertedImageUrl);
                    console.log('  - Tipo de conversión:', originalImageUrl.includes('drive.google.com') ? 'Google Drive' : 'Externa/Placeholder');
            ...
            console.log('✅ Productos procesados:', AppState.products.length, 'productos cargados');
            console.log('🛍️ Productos:', AppState.products);
    ...
    renderProducts: () => {
            ...
            return `
                <div class="product-card">
                    <img 
                        ...
                        alt="${product.name}" 
                        class="product-image"
                        onerror="this.onerror=null; this.src='https://via.placeholder.com/300x200/f0f0f0/666666?text=Sin+Imagen'; console.log('❌ Error cargando imagen:', '${product.image}');"
                        onload="console.log('✅ Imagen cargada correctamente:', '${product.image}');"
                    />
        ...
        console.log('🎉 Productos renderizados, mostrando contenedor...');
        productsContainer.classList.remove('hidden');
